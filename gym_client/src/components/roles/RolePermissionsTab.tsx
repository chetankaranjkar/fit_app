    ...
    <>
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        {!isAdding && editingId == null ? (
          <Button type="button" size="md" onClick={handleStartAdd}>
            + Add role
          </Button>
        ) : null}
      </div>
      ...
      {(isAdding || editingId != null) && (
        ...
        >
          <h3 className="mb-4 text-base font-semibold text-white">{isAdding ? 'New role' : 'Edit role'}</h3>
          ...
          {formError && (
            <p
              className="mt-3 rounded-lg border border-rose-500/40 bg-rose-500/10 px-3 py-2 text-sm text-rose-200"
              role="alert"
            >
              {formError}
            </p>
      ...
      {!loadingRoles && (
        <div className="overflow-x-auto">
          ...
          {roles.length === 0 && <p className="py-6 text-center text-slate-400">No roles yet. Add one above.</p>}
        </div>
      )}

      <div className="glass-card dashboard-card mt-6 rounded-2xl p-6">
