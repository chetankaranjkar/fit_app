...
export function PaymentsPage() {
  ...
  const { data: paymentsPage, isLoading, isFetching } = useQuery({
    queryKey: ['payments-paged', page, pageSize, reportFromDate, reportToDate],
    queryFn: async () =>
      (
        await paymentsService.getPaged({
          page,
          pageSize,
  ...
  const totalPayments = paymentsPage?.totalRecords ?? 0

  const { data: statsPayments = [] } = useQuery({
    queryKey: ['payments-stats', reportFromDate, reportToDate],
    queryFn: async () => {
      const { data } = await paymentsService.getPaged({
        page: 1,
        pageSize: 500,
        fromDate: reportFromDate,
        toDate: reportToDate,
  ...
  const { data: memberships = [] } = useQuery({
    queryKey: ['user-memberships-picker'],
    queryFn: async () => {
  ...
  const ensureInvoiceMutation = useMutation({
    ...
    onSuccess: (data) => {
      void queryClient.invalidateQueries({ queryKey: ['payments-paged'] })
  ...
  const paymentStats = useMemo(() => {
    ...
    const sum = statsPayments.reduce((s, p) => s + p.amount, 0)
    const avg = total ? sum / total : 0
    return { total, sum, avg }
  }, [statsPayments, totalPayments])
  ...
  const paymentColumns = useMemo<DataGridColumnDef<Payment>[]>(
    () => [
      {
        id: 'membership',
        header: 'Membership',
      ...
      },
      {
        ...
        cell: ({ row }) => <span className="font-medium text-white">{formatInr(row.amount)}</span>,
      },
      {
        id: 'date',
        header: 'Date',
      ...
      },
      {
        ...
        hideBelow: 'lg',
        cell: ({ row }) => {
          const expiry = getPaymentExpiryDate(row.paymentDate, row.membershipId)
      ...
      },
      {
        ...
        minWidth: 100,
        width: 110,
        accessorFn: (p) => p.paymentMode,
        cell: ({ value }) => <StatusBadge variant="info">{String(value)}</StatusBadge>,
      },
      {
        id: 'receipt',
        header: 'Receipt',
      ...
      },
      {
        ...
        cell: ({ row }) =>
          !canManagePayments ? (
            <span className="text-[10px] text-slate-500">No access</span>
          ) : row.invoiceId ? (
            <button
              type="button"
              onClick={() => {
                setInvoiceAutoGenerated(false)
                setViewInvoiceId(row.invoiceId!)
              }}
              className="rounded-lg border border-emerald-400/35 bg-emerald-500/10 px-2 py-0.5 text-[10px] font-semibold text-emerald-300 hover:bg-emerald-500/20"
            >
              Invoice
            </button>
          ) : (
            <button
              type="button"
              onClick={() => ensureInvoiceMutation.mutate(row.id)}
              disabled={ensureInvoiceMutation.isPending}
              className="rounded-lg border border-amber-400/35 bg-amber-500/10 px-2 py-0.5 text-[10px] font-semibold text-amber-300 disabled:opacity-50"
            >
              Generate
            </button>
          ),
    ...
    [canManagePayments, ensureInvoiceMutation.isPending, memberships],
  )
